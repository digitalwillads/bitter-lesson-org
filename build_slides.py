...
(function(){
  ...
  function jump(){
    ...
    deck.scrollTop = slides[want].offsetTop;
    deck.style.scrollBehavior = '';
    current = want; update();
    console.log('[slides] deep link to', want+1, slides[want].id, 'scrollTop', deck.scrollTop);
  }
  if (want > 0){ jump(); window.addEventListener('load', jump); }
  else update();
})();
</script>
</body>
